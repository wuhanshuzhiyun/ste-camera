		...
		<scroll-view class="methods" scroll-y>
			...
			<view class="section">
				...
				<view class="btn-row">
					...
					<button v-if="lastPhoto" class="btn btn-default" @click="savePhotoToAlbum">保存到相册</button>
			...
			<view v-if="lastPhoto" class="photo-preview">
				...
				<image
					...
					class="preview-image"
					mode="aspectFit"
					@click="previewFullPhoto"
				/>
				<text class="preview-path">{{ lastPhotoPath }}</text>
			</view>

			<!-- 上传示例：用拍照返回的文件路径直接 uploadFile，无需 uni.chooseMedia -->
			<view v-if="lastPhoto" class="upload-box">
				<view class="input-row">
					<text class="input-label">上传地址：</text>
					<input v-model="uploadUrl" class="input-field" placeholder="https://你的上传接口" />
				</view>
				<button class="btn btn-primary" @click="onUploadPhoto">上传图片（uni.uploadFile）</button>
				<text class="upload-tip">自定义相机已返回本地文件路径，直接 uploadFile 即可，无需调用 uni.chooseMedia（那会触发「未添加模块」报错）。</text>
			</view>
			</view>
			...
			<view class="section">
				...
				<view class="quality-row">
					...
					<slider :value="scanBarStartY" min="0" max="500" step="5" @change="scanBarStartY = $event.detail.value" class="quality-slider" />
				</view>

				<!-- 结束位置 -->
				<view class="quality-row">
					<text class="quality-label">结束Y（dp）：{{ scanBarEndY }}</text>
					<slider :value="scanBarEndY" min="0" max="800" step="5" @change="scanBarEndY = $event.detail.value" class="quality-slider" />
				</view>
...
const onTakePicture = () => {
	...
	log('正在拍照...');
	console.log('[all-test] calling takePicture');
	takePicture((path) => {
		console.log('[all-test] takePicture callback, path=', path);
		if (path && path.length > 0) {
			// 解析 Swift 层返回的 dataURL|||filePath 格式
			// dataURL 用于 <image> 预览，filePath 用于保存相册
			let displayPath = path;
			let savePath = path;
			if (path.indexOf('|||') >= 0) {
				const parts = path.split('|||');
				displayPath = parts[0];  // dataURL
				savePath = parts[1];     // 文件路径
				console.log('[all-test] dataURL length=', displayPath.length, ', filePath=', savePath);
			} else {
				const filePath = path.startsWith('file://') ? path : 'file://' + path;
				displayPath = filePath;
			}
			lastPhoto.value = displayPath;
			lastPhotoPath.value = savePath;
			log('拍照成功', 'success');
			uni.showToast({
				title: '拍照成功',
				icon: 'success'
		...
		takePicture((path) => {
			if (path && path.length > 0) {
				...
				if (path.indexOf('|||') >= 0) {
					...
					displayPath = path.startsWith('file://') ? path : 'file://' + path;
				}
				lastPhoto.value = displayPath;
				lastPhotoPath.value = savePath;
				log('水印拍照成功', 'success');
...
const previewFullPhoto = () => {
	// uni.previewImage 不支持 dataURL，需要用文件路径
	const filePath = lastPhotoPath.value.startsWith('file://') ? lastPhotoPath.value : 'file://' + lastPhotoPath.value;
	uni.previewImage({
		urls: [filePath],
		current: filePath
	});
...
/**
 * 保存最新照片到系统相册，用于确认图片实际内容
 */
const savePhotoToAlbum = () => {
	if (!lastPhotoPath.value) {
		uni.showToast({ title: '没有可保存的照片', icon: 'none' });
		return;
	}
	uni.saveImageToPhotosAlbum({
		filePath: lastPhotoPath.value,
		success: () => {
			log('已保存到相册：' + lastPhotoPath.value.split('/').pop(), 'success');
			uni.showToast({ title: '已保存到相册', icon: 'success' });
		},
		fail: (err) => {
			log('保存相册失败：' + JSON.stringify(err), 'error');
...
/**
 * 上传刚拍的照片。
 * 关键点：自定义相机已经把本地文件路径（lastPhotoPath）交给我们，
 * 直接交给 uni.uploadFile 即可，不要再调用 uni.chooseMedia/chooseImage 去「选图」，
 * 后者会触发「打包时未添加 uni-chooseMedia 模块」报错（需在 manifest.json 勾选 Camera&Gallery 模块）。
 */
const onUploadPhoto = () => {
	if (!lastPhotoPath.value) {
	...
	if (!uploadUrl.value) {
		uni.showToast({ title: '请先填写上传地址', icon: 'none' });
		return;
	}
	log('开始上传图片：' + lastPhotoPath.value.split('/').pop());
	console.log('[上传] filePath=', lastPhotoPath.value, ', url=', uploadUrl.value);
	uni.uploadFile({
		url: uploadUrl.value,
		filePath: lastPhotoPath.value,
		name: 'file',
		success: (res) => {
			log('上传成功，状态码=' + res.statusCode, 'success');
			console.log('[上传] 返回=', res.data);
			uni.showToast({ title: '上传成功', icon: 'success' });
		},
		fail: (err) => {
			log('上传失败：' + JSON.stringify(err), 'error');
...
const onShowCameraWithScanBar = async () => {
	...
	try {
		const { windowWidth, windowHeight } = await uni.getSystemInfo();
		// 清理 URL：去除首尾空格和反引号（用 split/join 避免正则中反引号被编译器误解析）
		const rawImageUrl = scanBarImage.value || '';
		const cleanImageUrl = rawImageUrl.trim().split(String.fromCharCode(96)).join('').trim();
		console.log('[all-test] rawImageUrl length=', rawImageUrl.length, ', cleanImageUrl length=', cleanImageUrl.length);
		console.log('[all-test] cleanImageUrl=', cleanImageUrl);
		const scanBarConfig = scanBarShow.value
			? {
					image: cleanImageUrl,
			...
			: undefined;
		console.log('[all-test] onShowCameraWithScanBar: scanBarShow=', scanBarShow.value);
		console.log('[all-test] onShowCameraWithScanBar: scanBarConfig=', JSON.stringify(scanBarConfig));
		showCamera({
			width: windowWidth,
			height: Math.floor(windowHeight * 0.6),
			top: 0,
